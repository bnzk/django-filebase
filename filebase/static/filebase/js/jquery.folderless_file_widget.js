...
(function($){
	$.fn.filebase_file_widget = function()
	{
		this.each(function(){
			var _self = $(this);
            var browse_button = _self.find(".filebase_browser");
            var upload_button = _self.find(".filebase_uploader");
            var upload_info = _self.find(".filebase_widget_upload_info");
            var remove_button = _self.find(".filebase_remove");
            var edit_button = _self.find(".filebase_edit");
            var unknown_img = _self.find(".filebase_unknown");
            var raw_id_field = _self.find(".filebase_raw_id_field input");
            var label = _self.find(".filebase_file_label");
            var thumb = _self.find(".filebase_thumb");
            var file_link = _self.find(".filebase_file_link");
            var file_input = _self.find(".filebase_fileinput");

            var ajax_request = false;

            var init = function() {
                _self.addClass("filebase_widget_initialized")
